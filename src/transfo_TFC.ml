...
let transform_event_tfc f ev =
  ...
  make_event ~ev_name:ev.ev_name ~ev_args:ev.ev_args
...
let convert ({ check = { chk_using; _ }; model } as ast) =
  let updated_events =
    ...
    | TFC f -> List.map (transform_event_tfc f) model.events
    | _ ->
        failwith
          "error in transfo_TFC.convert: specified transformation is not TFC"
  in
  let updated_model =
    make_model ~sorts:model.sorts ~relations:model.relations
      ~constants:model.constants ~closures:model.closures ~axioms:model.axioms
