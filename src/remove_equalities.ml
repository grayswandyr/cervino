...
let build_pred_eq_from_sort s =
  make_relation ~rel_name:(Name.create_from_name_and_prefix "_eq_" s) 
                ~rel_profile:[s;s] ()
...
| G (f) ->
  ...
  let remove_eq_fml_list =
    ...
    *)
    let equality_axiom_for_rel_at_i rel i =
      ...
      let aux_vars = List.foldi
        (fun list_acc i cur_sort -> 
            (List.cons (make_variable ~var_name:(Name.make_unloc ("_eq_var_"^ (string_of_int i))) ~var_sort:cur_sort ) list_acc)
        )
        []
        restricted_prof
      in
      ...
      let x_equals_y = lit @@ pos_app 0 (build_pred_eq_name_from_sort s) (List.cons term_x [term_y]) in
      all var_x (all var_y (implies (x_equals_y)
        (List.fold_right (fun cur_var cur_fml -> all cur_var cur_fml)
          vars_except_i
      ...
      let equality_axiom_for_rel_and_s rel s =
        let eq_axioms, _ = List.fold_left (fun (cur_list, cur_idx) cur_sort -> 
          if (equal_sort cur_sort s) then
            (List.cons (equality_axiom_for_rel_at_i rel cur_idx) cur_list, cur_idx+1)
          else
            (cur_list, cur_idx+1)
          )
          ([], 0)
  ...
  let remove_eq_model m = 
    let eq_sorts_axioms, updated_axioms = remove_eq_fml_list m.axioms in
    let eq_sorts_chk_fml, updated_chk_fml = remove_eq_fml m.check.chk_body in
    let eq_sorts_assuming, updated_assuming = remove_eq_fml m.check.chk_assuming in
    let updated_check = make_check ~chk_name:m.check.chk_name ~chk_body:updated_chk_fml ~chk_assuming:updated_assuming ~chk_using:m.check.chk_using in
    let eq_sorts = Sorts.union eq_sorts_axioms (Sorts.union eq_sorts_chk_fml eq_sorts_assuming) in 
    let relations_eq = Sorts.fold
      (fun s rel_list -> List.cons (build_pred_eq_from_sort s) rel_list)
      eq_sorts
      []
    in
    let equality_axioms = Sorts.fold
      (fun s cur_fmls -> (List.append (equality_axiom_for_rel_list_and_s m.relations s) cur_fmls))
      eq_sorts
      []
    in
    {m with
      relations = List.append m.relations relations_eq;
