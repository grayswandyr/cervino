...
let rec check_ufo_prime_inner
  ...
  match f with
  | Unop ((Always | After | Eventually), _) | Quant _ ->
      raise (Not_in_ufop f_loc)
  | Lit _ | Test _ ->
      ()
  | Unop (Not, f') ->
      check_ufo_prime_inner model f'
  | Binop (f1, _, f2) ->
  ...
  | Block b ->
      check_ufo_block check_ufo_prime_inner model b
  | Call (name, _) ->
      let body = find_pred_body model name in
      check_ufo_block check_ufo_prime_inner model body


and check_ufo_prime model (L.{ data = f; _ } as f_loc) =
  match f with
  | Quant (Some_, _, _) | Unop ((Always | After | Eventually), _) ->
      raise (Not_in_ufop f_loc)
  | Lit _ | Test _ ->
      ()
  | Unop (Not, f') ->
  ...
  | Quant (All, _, b) | Block b ->
      check_ufo_block check_ufo_prime model b
  | Call (name, _) ->
      let body = find_pred_body model name in
      check_ufo_block check_ufo_prime model body


and check_ufo_block proc model block = List.iter (proc model) block
...
let check_and_report model =
  ...
  | Not_in_ufop (L.{ loc; _ } as f) ->
      let msg =
        Format.(
          sprintf
            "%d:%d-...: formula forbidden in an event or predicate called from \
             an event@\n\
             %a"
            (L.begl loc)
            (L.begc loc)
