...
let rec instantiate_tprl constlist fml =
  ...
  | Exists (v, f) -> exists v (instantiate_tprl constlist f)
  | All (v, f) ->
  ...
  | F f -> eventually @@ instantiate_tprl constlist f
  | G f -> always @@ instantiate_tprl constlist f
...
let rec instantiate_ae constlist fml =
  ...
  | Exists (v, f) -> exists v (instantiate_ae constlist f)
  | All (v, f) ->
      let subfml = instantiate_ae constlist f in
      if includes_exists f then
        conj
        @@ List.map
             (fun cur_const -> substitute v ~by:cur_const subfml)
             constlist
      else all v subfml
  | F f -> eventually @@ instantiate_ae constlist f
  | G f -> always @@ instantiate_ae constlist f
...
let convert ast =
  ...
  in
  let updated_check =
    { ast.check with chk_assuming = updated_assuming }
  in
