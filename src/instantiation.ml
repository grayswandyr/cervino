...
let rec instantiate constlist fml =
  ...
  | (True | False | Lit _) as f ->
      f
  | And (f1, f2) ->
      and_ (instantiate constlist f1) (instantiate constlist f2)
  | Or (f1, f2) ->
      or_ (instantiate constlist f1) (instantiate constlist f2)
  | Exists (v, f) ->
      exists v (instantiate constlist f)
  | All (v, f) ->
  ...
  | F f ->
      eventually @@ instantiate constlist f
  | G f ->
      always @@ instantiate constlist f
