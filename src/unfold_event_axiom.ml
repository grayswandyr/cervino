...
let unfold_event_formula axioms =
  ...
  let constants : (variable * (constant * constant)) list ref = ref [] in
  (* retrieves (if they exist) or creates 2 fresh constants for a given bound variable *)
  let constants_for_bound_var ({ var_name; var_sort } as var) =
  ...
  let rec unfold (subst : (variable * term) list) fml =
    match fml with
    | Exists (_, v, f) ->
        let c1, c2 = constants_for_bound_var v in
        or_ (unfold ((v, cst c1) :: subst) f) (unfold ((v, cst c2) :: subst) f)
  ...
  let fresh_constants =
    List.flat_map (fun (_, (c1, c2)) -> [ c1; c2 ]) !constants
  in
