...
let main verbosity instantiate output_cervino property input output =
  ...
  try
    ...
    let result =
      if instantiate then Instantiation.convert ast else (Wf.check ast; Transfo.convert ast)
    in
    let pp = if output_cervino then Ast.Cervino.pp else Ast.Electrum.pp in
    match output with
    | None -> pp Fmt.stdout result
    | Some s ->
